...
export async function GET(request: Request) {
  try {
    ...
    // แก้ไขตรงนี้ - ใช้ string literal หรือ select method แทน
    const users = await User.find(query).select('-password'); // ไม่ดึง password
    
    console.log('Found users count:', users.length);
    if (users.length > 0) {
      console.log('First user fields:', Object.keys(users[0].toObject()));
      const firstUser = users[0] as any;
      console.log('Sample user userImage:', firstUser.userImage ? 'exists' : 'missing');
    }
