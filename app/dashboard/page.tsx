// app/dashboard/reports/page.tsx - แก้ไขส่วน renderDriverReport
'use client';
...
export default function ReportsPage() {
  ...
  // แก้ไขส่วน renderDriverReport - ลบคอลัมน์ปี้ และปรับการแสดงรายรับ
        ...
        <NeoCard className="p-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {reportData.drivers?.slice(0, 20).map((driver: any, index: number) => (
                  <tr key={index} className="border-b">
                    <td className="p-2 font-medium">{driver.name}</td>
                    <td className="p-2 text-gray-600">{driver.employeeId}</td>
                    ...
                    <td className="p-2 text-right">
                      <span className={`font-bold ${
                        driver.totalIncome > 0 ? 'text-green-600' : 'text-gray-400'
                      }`}>
                        ₭{driver.totalIncome?.toLocaleString() || 0}
                      </span>
                    </td>
                  </tr>
                ))}
          ...
          {reportData.drivers?.length > 20 && (
            <div className="mt-3 text-center text-sm text-gray-500">
              ແສດງ 20 ລາຍການແຮກ ຈາກທັງໝົດ {reportData.drivers.length} ຄົນ
            </div>
          )}
        </NeoCard>
      </div>
